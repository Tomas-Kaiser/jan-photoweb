...
// Hardcoded category order and display names
const CATEGORIES = [
    { slug: "brands-and-campaign", label: "Brands and Campaign" },
    { slug: "fashion-shows", label: "Fashion Shows" },
    { slug: "portraits", label: "Portraits" },
    { slug: "visual-stories", label: "Visual Stories" },
    { slug: "weddings", label: "Weddings" },
    { slug: "couples", label: "Couples" },
    { slug: "fashion-and-editorials", label: "Fashion and Editorials" },
]

const AlbumsPage = async () => {

    // Fetch all albums, grab first one per category as cover
    const allAlbums = await db
        .select()
        .from(albums)
        .orderBy(asc(albums.sortOrder))

    // Build a map: category slug → first album row
    const coverByCategory = new Map(
        allAlbums.map((a) => [a.category, a])
    )

    const albumsCategory = CATEGORIES
        .filter((cat) => coverByCategory.has(cat.slug))
        .map((cat) => {
            const cover = coverByCategory.get(cat.slug)!
            return {
                name: cat.label,
                imgSrc: cover.coverUrl,
                link: `/albums/${cat.slug}`,
